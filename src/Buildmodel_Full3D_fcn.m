...
%%
function Buildmodel_Full3D_fcn(HeartModel,filename,node_n,node_m,node_nm,path,probe,systempath)
...
for i=1:size(Node,1) %iterates through node list
    ...
    add_block('simulink/Ports & Subsystems/Model',myblock,'ModelName',node); %adds block to model canvas
...
for i=1:size(Path_name,1)    
    ...
    add_block('simulink/Ports & Subsystems/Model',myblock,'ModelName',path);
    set_param(myblock,'Position',[left_corner(1) left_corner(2) left_corner(1)+l left_corner(2)+w]);
...
for i=1:size(Path_name,1)  
    ...
    myblock=sprintf('%s/%s',Heart,block_name);
    add_block('simulink/Ports & Subsystems/Model',myblock,'ModelName',probe);
    set_param(myblock,'Position',[left_corner(1) left_corner(2) left_corner(1)+l left_corner(2)+w]);
...
for i=1:size(Node,1)
    ...
    % Use summation block if multiple inputs
     ...
     % add input tags to the joint      
      ...
      if strcmp(Node_name{i,1},'VEND150')
      ...
      if numIns==1 && ~strcmp(Node_name{i,1},'VEND150') && ~strcmp(Node_name{i,1},'AT066')
